...
class Mundo:
    ...
    def atualizacao_matriz(self):
        for i in range(self.tamanho_linhas):
            for j in range(self.tamanho_colunas):
                # Remove "FL" e "G" se presentes, caso contrário, define como "0" se o elemento for apenas "FL" ou "G"
                if "1" in self.matriz[i][j] or "G" in self.matriz[i][j]:
                    self.matriz[i][j] = self.matriz[i][j].replace("1", "").replace("G", "")
                    if self.matriz[i][j] == "":
                        self.matriz[i][j] = "0"
